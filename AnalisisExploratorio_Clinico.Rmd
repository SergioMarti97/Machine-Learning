...
lIsFactor <- sapply(intVars, function(v) {
  return(!(length(levels(as.factor(v))) > 4)) 
})
...
sprintf("Número de variables numéricas cuantitativas: %d", length(numVars))
sprintf("Número de variables enteras cuantitativas: %d", length(intVars[!lIsFactor]))
sprintf("Número de variables categóricas: %d", length(facVars))

# Añadimos a las variables numericas las variables enteras cuantitativas
numVars <- cbind(numVars, intVars[!lIsFactor])

# ----------------------------------------------------------------------

# Transformamos a factores las variables cualitativas
facVars[,1:ncol(facVars)] <- lapply(facVars[,1:ncol(facVars)], function(x) {
  myFactor <- factor(x)
  if (length(levels(myFactor)) < 4) { # RCB.category
    if (sum(levels(myFactor) != c("2", "3")) > 1) { # Grade.pre.chemotherapy
      if (sum(levels(myFactor) == c("-1", "1")) > 1) { # ER.status, HER2.status & LN.at.diagnosis
        x <- replace(x, x == -1, 0)
      }
...
# Mostramos las variables de tipo factor
str(facVars)
# summary(facVars)
...
rm(facVars, numVars, intVars, lIsFactor, dfClinicalVars)
```
...
html(describe(facVars))
```

Obtenemos que hay 54 variables númericas cuantitativas (44 reales y 11 enteras) y 17 variables de tipo factor cualitativas o categóricas. Además, las variables de tipo categoricas se pueden tratar como booleanas (verdadero o falso) porque solo toman dos valores: 0 o 1, o, -1 y 1. Excepto dos variables: "**Grade.pre.chemotherapy**" que toma valores de 2 o 3, y "**RCB.category**" con 4 niveles.
...
```{r}
ggplot2::ggplot(dfClinical, aes(x = resp.pCR)) + 
  ggplot2::geom_bar() + 
  ggplot2::ggtitle("resp.pCR")

# resp.Chemosensitive
ggplot2::ggplot(dfClinical, aes(x = resp.Chemosensitive)) + 
  ggplot2::geom_bar() + 
  ggplot2::ggtitle("resp.Chemosensitive")

# resp.Chemoresistant
ggplot2::ggplot(dfClinical, aes(x = resp.Chemoresistant)) + 
  ggplot2::geom_bar() + 
  ggplot2::ggtitle("resp.Chemoresistant")

# RCB.score
ggplot2::ggplot(dfClinical, aes(x = RCB.score)) + 
  ggplot2::geom_histogram() + 
  ggplot2::ggtitle("RCB.score")

# RCB.category
ggplot2::ggplot(dfClinical, aes(x = RCB.category)) + 
  ggplot2::geom_bar() + 
